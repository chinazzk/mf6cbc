module geometry_class
    ...
    subroutine read_inactive_cells_ (this,fname,ivar,const,flag,nx,ny,nz) 
       use functions, only: generate_unit
		 implicit none
    ...
    subroutine read_boundary_ (this,unit) 
		 ...
		 type(geometry_cl), intent(inout) :: this
	       ...
	       read(unit,*) this%ib(1,1),this%ib(1,2),this%ib(2,1),this%ib(2,2),this%ib(3,1),this%ib(3,2) ! line23 in .inp file
    end subroutine
...
!   ----------------------------------------------------------------------------------------
    function interpolate_array_ (this,array,cell,coord) result (value)
	   use array_class
	   use functions, only: interpolation_trilinear
	   implicit none
